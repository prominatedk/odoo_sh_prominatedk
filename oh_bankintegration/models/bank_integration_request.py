...
class BankIntegrationRequest(models.Model):
    ...
    def set_request_id(self):
        for record in self:
            record.write({'request_id': 'REQ_{company_id}_{timestamp}'.format(company_id=record.company_id.id, timestamp=datetime.datetime.now().timestamp())})
                    ...
                    if 'entries' in data:
                        ...
                        for transaction in lines:
                            ...
                            if round(transaction['balance'], 2) == round(current_balance_amount + transaction['amount'], 2):
                                ...
                                running_balance += round(transaction['amount'], 2)
                            else:
                        ...
                        _logger.warn('There are now transactions for journal {journal} in company {company} for period {from_date} - {to_date}'.format(journal=self.journal_id.name, company=self.company_id.display_name, from_date=from_date, to_date=to_date))
