...
class AccountMove(models.Model):
    ...
    bankintegration_payment_autopay = fields.Boolean(
        string='Auto pay', default=_default_bankintegration_payment_autopay, help="If this is set our scheduler automatic make payment")
    bankintegration_payment_journal_id = fields.Many2one(
        'account.journal', string='Payment journal', default=_default_bankintegration_payment_journal_id, domain=[('type', '=', 'bank')])
    payment_reference = fields.Char(string='Payment Reference')
    bankintegration_payment_error = fields.Char(string='Payment Error')
    bankintegration_request_id = fields.Char(string="Request id")
    bankintegration_payment_id = fields.Char(string="Payment id")
    bankintegration_payment_attempts = fields.Integer(string="Payment attempts", default=0)
    # FIK Payment code
    ...
    payment_fik_number_payment_string = fields.Char(string='FIK Payment String')
    # FIK Creditor number. The creditor FIK agreement number
    payment_fik_number_creditor = fields.Char(string='FIK Creditor')

    # This will be computed and stored
    payment_fik_number = fields.Char(string="FIK Number", compute='_compute_payment_fik_number', inverse='_compute_inverse_payment_fik_number', store=True)
    ...
    def _compute_inverse_payment_fik_number(self):
        for record in self:
            ...
            if not record.payment_fik_number:
                continue
            if not record.move_type == 'in_invoice':
                continue
    ...
    def _onchange_invoice_date_due(self):
        for record in self:
            if record.invoice_date_due:
                record.bankintegration_payment_date = record.invoice_date_due + \
                    datetime.timedelta(days=record.company_id.bankintegration_payment_margin)
    ...
    def action_reset_bankintegration_bankintegration_payment_status(self):
        ...
        if self.bankintegration_payment_status not in ['rejected', 'failed']:
            raise UserError(
                _('We do not allow resend of an invoice where the previous request has not returned an error or a rejection'))
        elif self.state != 'posted':
            raise UserError(_('Invoices must be posted in order to proceed'))
        return self.write({
    ...
    def cron_transfer_bankintegration_payments(self):
        for company in self.env['res.company'].search([]):
            ...
            if not company.bankintegration_payment_journal_id.has_valid_bankintegration_config():
                continue
            # Process all vendor bills in the company, which are to be paid
            # using bankintegration today
            vendor_bill_domain = self._get_bankintegration_vendorbill_domain(
                company)
    ...
    def _get_bankintegration_vendorbill_domain(self, company, journal):
        if company.bankintegration_send_on_validate:
            return [('bankintegration_payment_status', '=', False), ('move_type', '=', 'in_invoice'), ('state', '=', 'posted'), ('bankintegration_payment_autopay', '=', True), ('company_id', '=', company.id), ('bankintegration_payment_journal_id', '=', journal.id)]
        else:
            return [('bankintegration_payment_status', '=', False), ('move_type', '=', 'in_invoice'), ('state', '=', 'posted'), ('bankintegration_payment_date', '<=', fields.Date.today(self)), ('bankintegration_payment_autopay', '=', True), ('company_id', '=', company.id), ('bankintegration_payment_journal_id', '=', journal.id)]

    def _pay_with_bankintegration(self, journal, is_scheduler=False):
        BankIntegrationRequest = self.env['bank.integration.request']
        results = self._generate_bankintegration_transactions(
        ...
        for index, result in enumerate(results):
            if not result[2]:
                results.pop(index)
                _logger.error(
                    'During generation of payment data, there was an error generating payment details for vendor bill with ID {}. The payment has been removed from the list of payments to make. Please see previous log entries for details'.format(result[1]))
        if not results:
            _logger.error(
                'There are no bills to process. Please check the error messages on each bill')
            return False
        ...
        # Validate that all bills are paid from the same account and in the
        # same currency
    ...
    def _generate_bankintegration_transactions(self, is_scheduler=False):
        results = []
        for record in self:
            vals = record._generate_bankintegration_payment_transaction_data(
                is_scheduler=is_scheduler)
            if not vals:
                record.write({
                ...
                _logger.error('There was an error generating the transaction for vendor bill {bill}'.format(
                    bill=record.display_name))
    ...
    def _generate_bankintegration_payment_transaction_data(self, is_scheduler=False):
        ...
        # Legacy modification. If the invoice was created before the default
        # value of bankintegration_payment_attempts was set to 1, then it will be 0 and we have to
        # set it as one
        ...
        vals = {
            ...
            'paymentDate': self.bankintegration_payment_date.strftime('%Y-%m-%d') if self.bankintegration_payment_date else (self.invoice_date_due + datetime.timedelta(days=int(self.company_id.bankintegration_payment_margin))).strftime('%Y-%m-%d'),
            'paydate': self.bankintegration_payment_date.strftime('%Y%m%d') if self.bankintegration_payment_date else (self.invoice_date_due + datetime.timedelta(days=int(self.company_id.bankintegration_payment_margin))).strftime('%Y%m%d'),
            'paymentId': 'PAY_{bill}_{provider}_{id}_{count}'.format(bill=self.name, provider=self.company_id.bankintegration_erp_provider, id=self.id, count=self.bankintegration_payment_attempts),
            'text': self.name + ' ' + self.partner_id.display_name,
            'urgency': 1,
            'creditorText': self.payment_reference or self.ref or self.company_id.display_name,
        ...
        if self.payment_fik_number:
            ...
            if not self._payment_fik_number_is_valid():
                if is_scheduler:
                    ...
                    raise UserError(
                        _('The FI payment string on the vendor bill is not valid'))
            ...
            # If neither a FI payment string or a bank account has been
            # provided, then we look at the partner and use the first listed
            # account
            if self.partner_id.bank_ids.ids:
                # Filter bank accounts on the partner by sequence and get the
                # first one
                chosen_bank = self.partner_id.bank_ids.sorted(key='sequence')[
                    0]
                vals['account'] = chosen_bank.get_bban(
                ) if self.company_id.bankintegration_use_odoo_bban else chosen_bank.sanitized_acc_number
                ...
                if is_scheduler:
                    ...
                    raise ValidationError(
                        _('No bank accounts have been configured on the partner for this vendor bill'))

        # If no FI payment string or bank account has been found at this point,
        # we will not even get to this part of the code
        if not self._validate_bankintegration_required_fields(vals):
            error = _(
                'One or more required fields are missing in order to make the payment through bankintegration')
            if is_scheduler:
    ...
    def _payment_fik_number_is_valid(self):
        ...
        if not matches:
            # If not match is found for +01 or +73 syntax, validate if we have
            # the remaining
            matches = re.findall(
                r"(\+\d\d\<)(\d+)(\+)(\d+)(\<)", self.payment_fik_number)
            if not matches:
                # If still no matches are found, then the provided FI payment
                # string is not valid
                return False
        return True
