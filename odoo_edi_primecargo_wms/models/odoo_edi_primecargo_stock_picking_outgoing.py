...
class EdiStockPickingOutgoing(models.TransientModel):
    ...
    def prepare_document(self, document):
        vals = {
            ...
            'owner_code': document.company_id.primecargo_ownercode or self.env.user.company_id.primecargo_ownercode,
            'order_template_code': document.company_id.primecargo_template_code or self.env.user.company_id.primecargo_template_code,
            ...
            'shipping_product_code': document.primecargo_shipping_product_id.code or (document.company_id.primecargo_shipping_product_id.code or self.env.user.company_id.primecargo_shipping_product_id.code),
    ...
    def prepare_document_lines(self, line):
        prep_line = {
            ...
            'cost_currency_code': line.picking_id.company_id.currency_id.name or self.env.user.company_id.currency_id.name,
    ...
    def _update_pending(self, company, headers):
        ...
        for data in pending.json():
            picking = self.env['stock.picking'].search([('edi_document_guid', '=', data['uuid']), ('company_id','=', company.id), ('picking_type_code', '=', 'outgoing')], limit=1)
            if not picking.id:
            ...
            if not response_update.status_code == 200:
                _logger.warning('Response returned status code {}'.format(response_update.status_code))
    ...
    def _update_queue(self, company, headers):
        ...
        if len(queue.json()) > 0:
                ...
                if data['order_result_message'] == "OK":
                    ...
                    if picking.company_id.id:
                        if picking.company_id.primecargo_autovalidate_done:
                            self.autovalidate_picking(picking)
                    elif self.env.company_id.primecargo_autovalidate_done:
                        self.autovalidate_picking(picking)
            ...
            _logger.warning('There are no confirmed deliveries from PrimeCargo')

    def autovalidate_picking(self, picking):
        try:
