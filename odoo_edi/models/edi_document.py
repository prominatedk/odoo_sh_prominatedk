...
class EdiDocument(models.TransientModel):
    ...
    def send_document(self, document, document_res, endpoint):
        ...
        server_address = "{}".format(LIVE_API_ROOT if document_res.company_id.edi_mode == 'production' else TEST_API_ROOT) + endpoint
        settings = document_res.company_id
        token = settings.odoo_edi_token
        if not document_res.company_id.edi_mode == 'development':
            headers = {
    ...
    def validate_settings(self, document_res):
        ...
        if not self.env.user.company_id.country_id:
            raise UserError(_('The current company, %s, does not have a country set. This is required for EDI') % self.env.user.company_id.name)
