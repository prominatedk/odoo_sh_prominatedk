...
class ResConfigSettings(models.TransientModel):
    ...
    module_odoo_edi_sale = fields.Boolean(string='Enable EDI Sales orders and qoutations')
    module_odoo_edi_purchase = fields.Boolean(string='Enable EDI Purchase orders')
    edi_mode = fields.Selection(
        string="EDI Operation mode",
        help="Choose how we handle EDI connections. Option 'Production' for running against a production environment and sending real documents to real customers. Option 'Test' is to send documents to the testing environment where data is processed but not actually sent. Option 'Development/Debug' is used when we do not want to contact a server, which results in the file just being dumped locally on the server",
        related="company_id.edi_mode",
        readonly=False,
